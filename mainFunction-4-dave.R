...
tf2 <- data36[c(name.seePhase.phase,name.seePhase.temperature,name.vds.include,name.vds.temperature,name.vds.flowrate, name.vds.rh,name.vds.cO2, name.vds.gastype, name.vds.cCO2)]
...
if (process_type == '1'){
  ...
  for (i in 1:length(e1)){
    ...
    table_check2 <- dcast(e1[[i]], name.vds.cCO2 ~ name.vds.rh, length, value.var="Ph3")
...
###Works for nominal vds data###
  ...
  x_text = sub("AAAA1111",name.vds.cO2,x_text) #CHANGED NAME.VDS.RH TO O2 HERE ###NEED TO SOFT CODE THIS
  ...
  x_text2 = sub("AAAA1111",name.vds.cO2,x_text2) #CHANGED FROM RH FOR MY O2 FULL CALIBRATION ###NEED TO SOFT CODE THIS
  ...
  t_plot <- melt(table.selectedpoints.vds, id.vars = name.vds.cO2, variable.name = 'series') #CHANGED ID.VARS HERE FOR O2 FULL CALIBRATION ###NEED TO SOFT CODE THIS
  df <- t_plot
  x <- name.vds.cO2
  y <- "value"
  ...
  variable.series.plot <- ggplot(t_plot, aes_string(name.vds.cO2,"value")) + #CHANGED RH TO O2 HERE ###NEED TO SOFT CODE THIS
    geom_point(aes(colour = series)) + 
